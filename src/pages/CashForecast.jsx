    ...
    <ProtectedRoute>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Card><CardContent className="p-5"><div className="flex items-center gap-3"><div className={`rounded-xl p-2.5 ${forecast.lowest?.solde < 0 ? 'bg-rose-50 text-rose-700' : 'bg-blue-50 text-blue-600'}`}><Gauge className="h-5 w-5" /></div><div><p className="text-sm text-slate-500">Point bas prévisionnel</p><AmountDisplay amount={forecast.lowest?.solde || 0} size="lg" className={`font-bold ${forecast.lowest?.solde < 0 ? 'text-rose-700' : ''}`} /></div></div></CardContent></Card>
        </div>
